...
col_names <- c("Fmsy","pstar","tau","sigma_a",
               "sigma_p","H_weight","NCV_weight",
               "c1","c2","NCVshape","discount","Bhat","CV","VoI")

results <- load_VoI_data(file,col_names,p=0.1)
...
save(model, file="~/github/KalmanFilterPOMDPs/examples/data/VoI_Model.Rdata")
save(testing, file="~/github/KalmanFilterPOMDPs/examples/data/VoI_testing_data.Rdata")
save(training, file="~/github/KalmanFilterPOMDPs/examples/data/VoI_training_data.Rdata")
...
col_names <- c("Fmsy","pstar","tau","sigma_a","sigma_p","H_weight",
...
ggsave(file ="~/github/KalmanFilterPOMDPs/examples/figures/marginal_VoI_partial_dependence.png",
...
p <- ggplot(global_sensetivity_data,
  ...
  scale_y_discrete(labels = c(Fmsy=TeX("Productivtiy $(F^{MSY})$"),
                   ...
                   limits = factor(rev(c("CV","Bhat","H_weight","pstar","Fmsy",
                                         "c1","sigma_p","sigma_a","discount","NCV_weight",
                                         "c2","NCVshape","tau")))
  )+
...
ggsave(file ="~/github/KalmanFilterPOMDPs/examples/figures/marginal_VoI_sensetivity.png",
