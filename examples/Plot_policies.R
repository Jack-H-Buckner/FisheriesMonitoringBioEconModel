...
dat2 <- read.csv("~/github/KalmanFilterPOMDPs/examples/data/base_model_policy.csv")


dat_p <- perimeter(dat2)
E_log_B_vals = 1:150
V_log_B_vals = seq(0.01,0.61,0.005)
dat_p$inds0$E_log_B = E_log_B_vals[dat_p$inds0$y]
dat_p$inds0$V_log_B = V_log_B_vals[dat_p$inds0$x]
...
dat$E_log_B = E_log_B_vals[dat$y]
dat$V_log_B = V_log_B_vals[dat$x]


p<-ggplot(dat_p$inds0, aes(x=exp(log(E_log_B) + 0.5 * V_log_B) ,y = sqrt(exp(V_log_B)-1))) + 
...
p
ggsave(file = "~/github/KalmanFilterPOMDPs/examples/figures/Base_model_policy.png",
       p, height = 5, width = 7)
