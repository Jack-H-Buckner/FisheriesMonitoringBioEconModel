...
function deterministic(x,Ht,pars)
    ...
    Nt = log(exp(-F)*(1-m)*N + r*B*exp(v)/(1+b*B))
        
    Bt = log(exp(-F)*(1-m)*(N*winfty*(1-k)+k*B) + wr*r*B*exp(v-0.5*sigma_R^2)/(1+b*B))
    

    return [Nt, Bt, 0], (1-exp(-F))*(1-m)*(N*winfty*(1-k)+k*B)
...
function deterministic!(x,Ht,pars)
    ...
    x[1] = log(exp(-F)*(1-m)*N + r*B*exp(v)/(1+b*B))
        
    x[2] = log( exp(-F)*(1-m)*(N*winfty*(1-k)+k*B) + wr*r*B*exp(v-0.5*sigma_R^2)/(1+b*B))
