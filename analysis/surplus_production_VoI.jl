...
function EVoI!(acc,ind0,Fmsy,buffer,tau,sigma_a,sigma_p,H_weight,NCV_weight,c1,c2,b,discount,Nsamples)
    ...
    for n in 1:Nsamples
        x = next!(seq);CV = CVmin + (CVmax - CVmin)*x[1]; Bhat = Bmin + (Bmax - Bmin)*x[2]
        Var = log(CV^2+1); mu = log(Bhat) - 0.5*Var 
        acc[ind0+n,:] = [Fmsy,buffer,tau,sigma_a,sigma_p,H_weight,NCV_weight,c1,c2,b,discount,Bhat/Bmsy,CV,model.VoI([mu,Var])]
    end 
    return nothing  
end 
...
function main()
    Nmc = 2^9; Nsamples = 2^6 ; dims = 11
    acc = zeros(Nsamples*Nmc,dims+3);s = Sobol.SobolSeq(dims)
    for i in 1:Nmc
        print(i," ")
