...
function assessment_frequency!(Fmsy,buffer,tau,sigma_a,sigma_p,H_weight,NCV_weight,c1,c2,b,discount)

    model = SurplusProduction.init_simpler(
                Fmsy,buffer, tau, sigma_a, sigma_p, H_weight, NCV_weight, 
...
function main()
    ...
    for n in 1:Nmc
        ...
        omega = assessment_frequency!(Fmsy,buffer,tau,sigma_a,sigma_p,H_weight,NCV_weight,c1,c2,b,discount)
        acc[n,:] = [Fmsy,buffer,tau,sigma_a,sigma_p,H_weight,NCV_weight,c1,c2,b,discount,omega]
    end 
    return acc
end 
...
dat = main()
CSV.write("data/assessment_frequency.csv",Tables.table(dat);sep=',')
